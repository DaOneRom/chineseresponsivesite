(function($) {
	$.fn.slide = function(options) {
        /* Default Settings
		 * Start:true or false defaults to 1 7 to 1 7
		 * speed? 0 Rotation Speed default 2000 units: MS
		 * Animate: "Horizontal" | | " Opacity "Horizontal for transverse wheel okra 1 7 opacity for gradient Elimination 1 7
		 */
		this.defaults = {
			start: true,
			speed: 5000,
		...
		this.each(function() {
			...
			// $(this).find('.next').on('mouseover', function() {
			// 	var old = index;
			// 	if (index >= count - 1) {
			// 		index = 0;
			// 	}
			...
			// });
            //
			// $(this).find('.prev').on('mouseover', function() {
			//     var old = index;
			// 	if (index <= 0) {
			// 		index = count - 1;
			// 	}
			...
			$(this).find('.slide-tabs li').each(function(aIndex) {
				$(this).on('mouseover.slide-tabs', function() {
					if (aIndex < index) {
						change.call(_this, aIndex, index, 'fadeIn');
					}
